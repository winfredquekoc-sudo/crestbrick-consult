...
**Canonical copy**: this file supersedes the older `wa-bridge-restart.md` in the same directory (kept for history)
...
| Python MCP server | stdio subprocess that lets Claude read messages and contacts. Spawned by the Claude app. | Claude app process | Only when Claude reopens a session |

The Go bridge is extremely stable. If you or another agent are experiencing WhatsApp tool
failures inside a Claude session, the MCP server (Python process) is almost certainly what
dropped, not the bridge.
...
| Backup binary (pre-2026-04-25) | `/Users/winfredquek/whatsapp-mcp/whatsapp-bridge/whatsapp-bridge.backup-20260425` |
...
| HTTP watchdog plist | `~/Library/LaunchAgents/com.crestbrick.wa-bridge-http-check.plist` |
| HTTP watchdog script | `/Users/winfredquek/.claude/bin/wa-bridge-http-check.sh` |
| HTTP watchdog state file | `/Users/winfredquek/.claude/state/wa-bridge-http-state.txt` |
| Telegram env | `/Users/winfredquek/.telegram-bot.env` |
| REST API | `http://127.0.0.1:8080` |
| v4 intake runner | `/Users/winfredquek/crestbrick-consult/src/wa-pipeline/wa_intake_runner.py` |
| Intake plist (active sender) | `~/Library/LaunchAgents/com.crestbrick.wa-intake.plist` |
| Intake runner log | `/Users/winfredquek/.claude/state/listing-templates/runner.out.log` |
| Old pipeline plist (disabled) | `~/Library/LaunchAgents/com.crestbrick.wa-pipeline.plist.disabled` |
...
as a linked device, exposes a local REST API on `127.0.0.1:8080`, and writes all messages
to `store/messages.db`.

**Single sender doctrine (always enforced):** Only `com.crestbrick.wa-intake` (running
`wa_intake_runner.py` every 60 seconds) is the authorised outbound sender. The old
`com.crestbrick.wa-pipeline` plist is `.disabled` and the n8n autoreply workflow is
deactivated. Do not re-enable either. Never start a second process that calls
`http://127.0.0.1:8080/api/send` independently.
...
The bridge itself runs with `KeepAlive true` and `ThrottleInterval 30`, so crash restarts
are automatic within 30 seconds. The HTTP watchdog (`com.crestbrick.wa-bridge-http-check`)
runs every 3600 seconds and sends a Telegram alert on any healthy-to-down or
down-to-healthy transition. The watchdog alerts; launchd restarts.

**Pausing without a restart:** If you need to pause outbound processing without taking
down the bridge (e.g. during a maintenance window), send `/hold` or `/pause` via Telegram
to @Maddieprop_bot. This stops the intake pipeline from sending replies without
...
only to identify which recovery path in section 6 to follow.
...
- `HTTP 400` or `HTTP 422` — server is up (bad payload is the expected response)
...
```
Device logged out, please scan QR code to log in again   -> section 6b
405 client-outdated                                       -> section 6c
listen tcp 127.0.0.1:8080: bind: address already in use  -> section 6d
database is locked                                        -> section 6e
unable to open database file                              -> section 6e
stream:error (with code 401 or 403)                      -> section 7 (security — do not restart)
...
If `Scan this QR code` appears instead, the session was wiped. Go to section 6b.
...
as a temporary fallback while you investigate:
...
## 4. Automated recovery (current production config)
...
| `WorkingDirectory` | `/Users/winfredquek/whatsapp-mcp/whatsapp-bridge` | Binary resolves `store/` relative to this path |
...
The HTTP watchdog (`com.crestbrick.wa-bridge-http-check`) complements launchd:
launchd handles process-exit crashes; the watchdog catches silent disconnects where the
process is alive but not delivering messages. The watchdog sends Telegram alerts but does
NOT auto-restart the bridge.
...
```zsh
# Replace +65XXXXXXXX with Winfred's actual number
...
Also confirm the v4 intake runner is still active after the restart:
...
# 2. Wipe the stale session (it is invalid — the credentials are revoked)
...
| `messages.db` | Message history cache used by the v4 intake runner | Yes — loss of history only, no session impact |
...
| `405 client-outdated` returns immediately after a fresh whatsmeow rebuild | Breaking upstream API change requiring source edits to `main.go`. Stop the service; escalate to developer agent. |
...
`http://localhost:8080/api/send`, writes state to
`~/.claude/state/wa-bridge-http-state.txt`, and sends a Telegram alert on any
healthy-to-down or down-to-healthy transition.
...
Run the watchdog manually to force an immediate health check or test the Telegram alert:
...
### Single-sender discipline

Only `com.crestbrick.wa-intake` sends messages. The `wa-pipeline.plist.disabled` file and
the deactivated n8n autoreply workflow must not be re-enabled without confirming they will
not send duplicate replies alongside the active runner. Before any pipeline change, use
`/hold` or `/pause` via Telegram to pause outbound processing first.

If `wa_pipeline_live.py` `deliver()` logs `Send failed` errors, that is a bridge-down
symptom: the intake runner will self-retry 3 times with 2-second delays and then Telegram-
alert Winfred. You do not need to intervene on the Python side — fix the bridge.
...
Need to pause without restarting?
        Send /hold or /pause via Telegram -> pauses v4 intake runner, bridge stays connected
```
