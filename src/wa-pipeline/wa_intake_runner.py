...
def run():
    ...
    # Engine integrity pin (9 Sep 2026): the working tree IS production, and another session's
    # git reset silently reverted a deploy today. If the bytes on disk stop matching the pinned
    # committed state, hold every send and alarm; a missing pin only logs (first deploy).
    try:
