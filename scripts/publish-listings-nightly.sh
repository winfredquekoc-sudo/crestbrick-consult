#!/usr/bin/env bash
# publish-listings-nightly.sh — daily publisher for public/listings.json, modeled on
# the insights-drip publisher (~/.claude/bin/insights-drip.sh): regenerates the listings
# feed from Winfred's own landlord DB (scripts/gen-website-listings.py) into a clean
# origin/main worktree, commits ONLY public/listings.json when it changed, pushes
# (Vercel deploys on push to main). Idempotent — safe to run more than once a day.
#
