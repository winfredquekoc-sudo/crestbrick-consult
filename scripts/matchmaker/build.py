...
CRM_TEST = os.path.join(WORKTREE_ROOT, "tests/matchmaker/crm.test.mjs")
CRM_VALIDATE_TEST = os.path.join(WORKTREE_ROOT, "tests/matchmaker/crm-validate.test.mjs")
...
def run_tests():
    missing = [p for p in (SCORING_TEST, STATE_TEST, FILTERS_TEST, ROWS_TEST, CRM_TEST, CRM_VALIDATE_TEST, EXPORT_TEST, STATE_PY_TEST) if not os.path.exists(p)]
    if missing:
    ...
    for path in (SCORING_TEST, STATE_TEST, FILTERS_TEST, ROWS_TEST, CRM_TEST, CRM_VALIDATE_TEST):
