...
Ported from the monolith lineage (matchmaker/crm-cloud-backend:scripts/matchmaker/
revival_board.py) onto the v2 export lane's own helpers (export_data.py)."""
import datetime
import json
import os
import sys
...
def load():
    ...
    return ten["tenants"], land["landlords"], dist_area
...
def best_match(t, avail):
    # Only a handful of listings are live citywide -- "some listing passes the
    # budget floor" is not the same as "actually a fit", so this returns a
    # quality TIER, not just a pick.
...
def build_rows(tenants, avail, today, area_keywords):
    rows = []
    for t in tenants:
        ...
        district = (t.get("district") or (pd_raw[0] if pd_raw else "")
                    or infer_district("", pd_raw, t.get("preferred_location"), area_keywords))
...
def main():
    tenants, landlords, dist_area = load()
    today = datetime.date.today()
    area_keywords = build_area_keywords(dist_area)
    avail = [{
    ...
    rows = build_rows(tenants, avail, today, area_keywords)
