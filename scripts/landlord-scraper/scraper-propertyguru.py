#!/usr/bin/env python3
"""
scraper-propertyguru.py — PropertyGuru landlord rental scraper for prospecting.
...
STATE_FILE = SCRIPT_DIR / "scraper-propertyguru-state.json"
...
log_file = LOGS_DIR / f"propertyguru-{datetime.now().strftime('%Y-%m-%d')}.log"
logging.basicConfig(
    level=logging.INFO,
...
class PropertyGuruScraper:
    """Scrape PropertyGuru for individual landlord rental listings."""

    PG_RENTAL_URL = "https://www.propertyguru.com.sg/property-for-rent"
    ...
    def scrape(self) -> List[Dict]:
        ...
        logger.info(f"Scraping PropertyGuru rental listings")
        listings = []

        try:
            # Add filter params to get individual listings only
            url = self.PG_RENTAL_URL + "?agent_type=Individual"
            ...
            # PropertyGuru listing cards: look for common property listing containers
            cards = soup.find_all("div", class_=re.compile(r"listing|card|property-item", re.I))
    ...
    def _parse_card(self, card) -> Optional[Dict]:
        try:
            ...
            if listing_url and not listing_url.startswith("http"):
                listing_url = "https://www.propertyguru.com.sg" + listing_url

            seller_elem = card.find("div", class_=re.compile(r"seller|agent|author", re.I))
            name = None
            phone = None
    ...
    def queue_messages(self, listings: List[Dict]) -> int:
        ...
        for listing in listings:
            try:
                msg = (
                    f"Hi {listing['name']}, I saw your rental on PropertyGuru for SGD {listing['rent_sgd']}/month in {listing['location']}.\n\n"
                    f"I have qualified tenants ready and can also co-list on 99.co and my network for faster, better results. "
                    f"No upfront fees.\n\n"
                    f"Interested in a quick chat?\n\n"
        ...
        try:
            ...
            logger.info(f"Saved {queued_count} PropertyGuru messages to queue")
        except Exception as e:
...
def main():
    logger.info("=== PropertyGuru Landlord Scraper ===")
    scraper = PropertyGuruScraper()
    listings = scraper.scrape()
    if listings:
        queued = scraper.queue_messages(listings)
        logger.info(f"Queued {queued} messages from PropertyGuru")
    else:
        logger.info("No new PropertyGuru listings found")
